...
$query_gallery = "SELECT id, name, uid, albumid FROM image WHERE image.albumid = '".$albumid."'";
...
$row_gallery = mysql_fetch_assoc($gallery);
	...
	mysql_select_db($database_picweb, $picweb);
	$query_albumcheck = "SELECT user_album.name as name, user.first_name as first_name, user.last_name as last_name FROM user_album, user WHERE user.uid > '0' AND user_album.public = 'true' AND user_album.uid = user.uid AND user_album.id = '".$albumid."'";
...
$userFullName = $row_albumcheck['first_name']." ".$row_albumcheck['last_name'];

mysql_select_db($database_picweb, $picweb);
$query_albums = "SELECT * FROM user_album, user WHERE public = 'true' AND user.uid > '0' AND user_album.uid = user.uid ORDER BY name ASC";
...
<title>PicWeb - Public Albums Gallery</title>
</head>
...
<table class="bodyTable" cellspacing="0" cellpadding="4">
  <tr>
    <th class="tableTD"><strong>Public Albums</strong></th>
  </tr>
  <?php $color="1"; ?>
  <?php do { ?>
    <tr class="row<?php echo $color;?>">
      <td class="tableTD">
           <a href="public.php?albumid=<?php echo $row_albums['id']; ?>"><?php echo $row_albums['name']; ?></a>
      </td>
    </tr>
    <?php if($color == "1"){
			$color="2";
...
<?php }else{ 
	echo "<br />There are no public albums<br />";}?><br />
</div></td>

<td width="84%" align="center" valign="top">
<?php if (($totalRows_albumcheck) > '0'){
?>
<strong>Album: <?php echo $row_albumcheck['name']; ?></strong><br />
<strong>By: <?php echo $userFullName; ?></strong><br />
<table class="galleryTable" cellspacing="0" cellpadding="4">
<tr>
<?php if (($totalRows_gallery) > '0') {?>
	<?php $count = 0; ?>
    <?php do {?>
      <?php if ($count > 4) { echo "</tr><tr>"; $count = 0;}?><td><a href="publicget.php?pic=<?php echo $row_gallery['id']; ?>"><img src="thumb.php?id=<?php echo $row_gallery['id']; ?>"></a><br /><a href="publicget.php?pic=<?php echo $row_gallery['id']; ?>"><?php echo $row_gallery['name']; ?></a><?php $count = $count + 1; ?></td>
